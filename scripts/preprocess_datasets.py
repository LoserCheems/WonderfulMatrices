...
if __name__ == '__main__':
    ...
    dataset = dataset.select(range(fineweb_edu_size)).map(process_python_edu, input_columns=['text'], remove_columns=['text', 'id', 'metadata'], num_proc=args.num_proc)
    print(dataset)
    ...
    dataset = dataset.select(range(cosmopedia_v2_size)).map(process_cosmopedia, input_columns=['prompt', 'text'], remove_columns=['prompt', 'text', 'token_length', 'audience', 'format', 'seed_data'], num_proc=args.num_proc)
    print(dataset)
    ...
    dataset = dataset.select(range(python_edu_size)).map(process_python_edu, input_columns=['text'], remove_columns=['text', 'download_success', 'blob_id', 'repo_name', 'path', 'length_bytes', 'score', 'int_score'], num_proc=args.num_proc)
    print(dataset)
    ...
    dataset = dataset.select(range(open_web_math_size)).map(process_open_web_math, input_columns=['text'], remove_columns=['text', 'url', 'date', 'metadata'], num_proc=args.num_proc)
    print(dataset)
