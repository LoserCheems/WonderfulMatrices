...
def main(args):
    ...
    fineweb_edu_size = int(args.examples * fineweb_edu_ratio)
    cosmopedia_v2_size = int(args.examples * cosmopedia_v2_ratio)
    python_edu_size = int(args.examples * python_edu_ratio)
    open_web_math_size = int(args.examples * open_web_math_ratio)

    # 加载分词器
    # Load tokenizer
    old_tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_path)
    ...
    def get_training_corpus():
        for i in range(0, args.examples, 1000):
            samples = dataset[i : i + 1000]["examples"]
            yield samples
    
    # 训练分词器
    # Train tokenizer
    new_tokenizer = old_tokenizer.train_new_from_iterator(get_training_corpus, vocab_size=32768)
...
if __name__ == '__main__':
    ...
    argparser.add_argument("--num_proc", type=int, default=4)
